...
creditDataSerhat2 <- subset(creditDataSerhat,select=-c(inq_last_12m,total_cu_tl,inq_fi,total_rev_hi_lim,all_util,
...
max(table(creditDataSerhat2$title))
HäufigkeitTitle <- round(prop.table(table(creditDataSerhat2$title))*100)
tabelle <- cbind(HäufigkeitTitle, HäufigkeitTitle2)
