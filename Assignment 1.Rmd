...
sorted_frequency <- sort(kategorie_haeufigkeit, decreasing = TRUE)
top30 <- head(sorted_frequency, 30)
top30_namen <- names(top30)
creditData6$title <- ifelse(creditData2$title %in% top30_namen, creditData5$title, 'others')
```

Now we can see which categories are left:
```{r message = FALSE, warning = FALSE}
table(creditData6$title)
```

Finally, we can factorise this variable
```{r message = FALSE, warning = FALSE}
creditData6$title <- factor("title")
```
...
```{r message = FALSE, warning = FALSE}
summary(creditDataSerhat6$zip_code)
```
