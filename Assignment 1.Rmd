...
```{r message = FALSE, warning = FALSE}
plot(creditData5$revol_util, creditData5$revol_util,
     xlab = "revol_util", 
     ylab = "revol_util",
...
```{r message = FALSE, warning = FALSE}
creditData5 <- subset(creditData5, revol_util <= 200)
```
...
Finally, we can factorise this variable
```{r message = FALSE, warning = FALSE}
creditData5$combo_zip_addr <- factor("combo_zip_addr")
```

## Attribute acc_now_deling
Let us first have a look at the variable:
```{r message = FALSE, warning = FALSE}
summary(creditData5$acc_now_delinq)
```
...
```{r message = FALSE, warning = FALSE}
Higher0 <- sum(creditData5$acc_now_delinq > 0, na.rm = TRUE)
print(Higher0)
```

We should also check if there are any outliers:
```{r message = FALSE, warning = FALSE}
plot(creditData5$acc_now_delinq, creditData5$acc_now_delinq,
     xlab = "acc_now_delinq", 
     ylab = "acc_now_delinq",
...
```{r message = FALSE, warning = FALSE}
zero_in_column <- sum(creditData5$tot_coll_amt == 0, na.rm = TRUE)
...
```{r message = FALSE, warning = FALSE}
median_tot_coll_amt <- median(creditData5$tot_coll_amt, na.rm = TRUE)

creditData5$tot_coll_amt[is.na(creditData5$tot_coll_amt)] <- median_tot_coll_amt
```

We should also check if there are any outliers:
```{r message = FALSE, warning = FALSE}
plot(creditData5$tot_coll_amt, creditData5$tot_coll_amt,
     xlab = "tot_coll_amt", 
     ylab = "tot_coll_amt",
...
```{r message = FALSE, warning = FALSE}
creditData5 <- subset(creditData5, tot_coll_amt <= 2000000)
```
...
```{r message = FALSE, warning = FALSE}
zero_in_column <- sum(creditData5$tot_cur_bal == 0, na.rm = TRUE)
print(zero_in_column)
```
...
```{r message = FALSE, warning = FALSE}
mean_tot_cur_bal <- mean(creditData5$tot_cur_bal, na.rm = TRUE)

creditData5$tot_cur_bal[is.na(creditData5$tot_cur_bal)] <- mean_tot_cur_bal
```

We should also check if there are any outliers:
```{r message = FALSE, warning = FALSE}
plot(creditData5$tot_cur_bal, creditData5$tot_cur_bal,
     xlab = "tot_cur_bal", 
     ylab = "tot_cur_bal",
...
```{r message = FALSE, warning = FALSE}
creditData5 <- subset(creditData5, tot_cur_bal <= 4000000)
```
...
```{r message = FALSE, warning = FALSE}
creditData$addr_state <- as.factor(creditData$addr_state)
str(creditData$addr_state)
```
...
```{r message = FALSE, warning = FALSE}
creditData <- subset(creditData, is.na(creditData$annual_inc) != 1)
creditData$annual_inc <- if_else(creditData$annual_inc > 800000, 65000, creditData$annual_inc)
```
...
```{r message = FALSE, warning = FALSE}
creditData$application_type <- as.factor(creditData$application_type)
str(creditData$application_type)
```
...
```{r message = FALSE, warning = FALSE}
creditData <- subset(creditData, is.na(creditData$collections_12_mths_ex_med) != 1)
```
...
```{r message = FALSE, warning = FALSE}
creditData <- subset(creditData, is.na(creditData$delinq_2yrs) != 1)
```
...
```{r message = FALSE, warning = FALSE}
creditData <- subset(creditData, creditData$dti < 100)
```
